...
4. If all three fields were found, renames (or copies) the file to `YEAR_Author_Journal.pdf`, sanitizing the name and avoiding overwrites.
5. **If any field couldn't be determined, the file is left untouched** (no guessing, no partial renames) — unless you pass `--force`, in which case it renames anyway using `Unknown*` placeholders for whatever's missing.
...
# Rename even if some fields couldn't be found (fills gaps with Unknown*)
rename-paper --force ~/papers/
...
| `--force` | Rename even if author/year/journal couldn't all be determined, using `Unknown*` placeholders for missing fields. Without this flag, files with incomplete metadata are left completely untouched. |
...
By default, if **any** of author, year, or journal remains unknown after all eight strategies have run, **the file is left completely untouched** — nothing is renamed, nothing is guessed. The tool tells you exactly which field(s) it couldn't determine:

```
[skip] could not determine: journal — leaving file untouched
      (use --force to rename anyway with 'Unknown*' placeholders)
```

If you'd rather rename everything regardless — accepting placeholders like `UnknownJournal` for whatever's missing — pass `--force`. This restores the older, more permissive behavior.

When processing a folder, a summary line at the end reports how many files were skipped:

```
3 file(s) skipped due to incomplete metadata (re-run with --force to rename them anyway).
```
...
If no strategy manages to fill in every field, the file is skipped rather than renamed:

```
  [doi]      found DOI: 10.1234/example.doi.2021
  [doi]      Crossref returned 403
  [title]    guessed: "Deep Learning Approaches for Climate Prediction Models"
  [heuristic] (low confidence) -> {'author': 'Smith', 'year': '2021', 'journal': None}
  Final metadata: author='anonymous', year='2021', journal=None
  [skip] could not determine: journal — leaving file untouched
        (use --force to rename anyway with 'Unknown*' placeholders)
```
...
| File keeps getting skipped, never renamed | Some field couldn't be determined — check the `[skip] could not determine: ...` line to see which one, and why (scanned PDF? unusual layout? no DOI/arXiv/PMID?). Use `--force` if you're fine with `Unknown*` placeholders. |
...
- Use `--copy` instead of an in-place rename if you want to keep the originals untouched while testing.# rename_paper.py
